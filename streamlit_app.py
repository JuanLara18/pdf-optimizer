...
def main():
    ...
    uploaded_file = st.file_uploader(
        "Choose a PDF file",
        type="pdf",
        help="Upload a PDF file to compress"
    )
    ...
    if uploaded_file is not None:
        ...
        mode = st.radio(
            "Select mode:",
            ["🚀 Quick Compress", "⚙️ Advanced Options"],
            horizontal=True
        )
        ...
        if mode == "🚀 Quick Compress":
            ...
            with col2:
                aggressive = st.checkbox(
                    "Aggressive mode",
                    help="Require higher compression ratios"
                )
                
                keep_text = st.checkbox(
                    "Preserve text",
                    help="Never rasterize pages (keeps searchable text)"
                )
